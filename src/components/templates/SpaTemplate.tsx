...
export default function SpaTemplate({ business, services = [] }: SpaTemplateProps) {
  ...
  const defaultServices: Service[] = [
    { id: '1', name: 'Masaje Relajante', price: 120000, duration: '60 min', available: true },
    { id: '2', name: 'Masaje Descontracturante', price: 150000, duration: '60 min', available: true },
    { id: '3', name: 'Facial Hidratante', price: 95000, duration: '45 min', available: true },
    { id: '4', name: 'Aromaterapia', price: 80000, duration: '30 min', available: true },
    { id: '5', name: 'Paquete Relax Total', price: 250000, duration: '120 min', available: false },
  ]
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-teal-900 via-cyan-800 to-emerald-800">
      <div className="max-w-md mx-auto px-4 py-8">
        <motion.div className="text-center mb-6" initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }}>
          {business.logoUrl ? (
            <img src={business.logoUrl} alt={business.name} className="w-24 h-24 rounded-full mx-auto mb-4 border-4 border-teal-400 object-cover" />
          ) : (
            <div className="w-24 h-24 rounded-full mx-auto mb-4 bg-gradient-to-br from-teal-500 to-cyan-500 flex items-center justify-center border-4 border-white/20">
              <span className="text-4xl">🧘</span>
            </div>
          )}
          <h1 className="text-2xl font-bold text-white mb-1">{business.name}</h1>
          <p className="text-teal-200 text-sm mb-3">{business.description || 'Spa & Bienestar'}</p>
          <div className="text-sm"><span className="text-teal-100 flex items-center justify-center gap-1"><span>📍</span>{business.address}</span></div>
          {business.rating && (
            <div className="flex items-center justify-center gap-1 mt-2">
              <span className="text-yellow-400">⭐</span>
              <span className="text-white font-medium">{business.rating}</span>
              {business.reviewCount && <span className="text-teal-200">({business.reviewCount} reviews)</span>}
            </div>
          )}
        </motion.div>
        ...
        <motion.div className="space-y-3 mb-8" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }}>
          <a href="#servicios" className="flex items-center justify-between w-full p-4 bg-white/10 hover:bg-white/20 rounded-xl border border-white/20 transition-all group">
            ...
            <span className="text-white/50 group-hover:text-teal-400">→</span>
          </a>
          <a href={whatsappUrl('Hola, quiero agendar una cita para [servicio]')} target="_blank" rel="noopener noreferrer" className="flex items-center justify-between w-full p-4 bg-teal-600 hover:bg-teal-500 rounded-xl transition-all group">
            ...
            <span className="text-teal-200 group-hover:text-white">→</span>
          </a>
        ...
        <motion.div id="servicios" className="bg-white/10 rounded-2xl p-5 border border-white/20" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}>
            ...
            {availableServices.map((service) => (
              <div key={service.id} className="flex items-center justify-between p-3 bg-white/5 rounded-lg">
                <div>
                  ...
                  {service.duration && <p className="text-teal-300/60 text-xs">{service.duration}</p>}
                </div>
                <span className="text-teal-400 font-bold">{formatPrice(service.price)}</span>
              </div>
            ))}
          ...
          <a href={whatsappUrl('Hola, quiero agendar una cita')} target="_blank" rel="noopener noreferrer" className="mt-4 w-full block text-center p-3 bg-teal-600 hover:bg-teal-500 text-white font-bold rounded-xl transition-all">Reservar Ahora</a>
        </motion.div>

        <motion.div className="mt-6 text-center" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.6 }}>
          <p className="text-teal-200/70 text-sm flex items-center justify-center gap-2"><span>🕒</span>{business.hours}</p>
        </motion.div>
